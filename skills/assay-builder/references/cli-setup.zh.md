...
与项目注册表不同,每个工作区有一份位于 `.assay/systems-registry.json` 的系统注册表,自布局 v3 引入。用 `system` 命令组管理它,不要直接编辑 JSON:
