...
Layout v3 introduces `.assay/systems-registry.json` and per-system `system.yaml` contracts. When `migrate-layout` detects a v2 workspace (manifest exists, registry does not), the plan adds these step types:
