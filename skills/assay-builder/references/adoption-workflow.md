...
If the target already has a `.assay/manifest.json`, the CLI refuses to adopt — use `update` or `migrate-layout` instead. If it has a v2 manifest but no `systems-registry.json`, run `migrate-layout --dry-run` first to plan the v2→v3 upgrade.
