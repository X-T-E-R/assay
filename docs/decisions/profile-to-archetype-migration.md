...
Assay now models workspace shape with `project.archetype` and `project.mode` in `.framework/manifest.json`.
...
| `project.mode` | `learning`, `absorption` | Selects where `assay absorb` lands source material. |
...
- Deprecated profile aliases are accepted only for old workspaces; new docs and commands should use `--archetype`.
- Legacy standalone runtime configuration is no longer generated or read.
...
1. Run `assay migrate-layout --root <workspace> --dry-run`.
2. Review any v2→v3 steps that create `.framework/systems-registry.json` or generate `systems/<name>/system.yaml`.
3. Apply with `assay migrate-layout --root <workspace> --apply` only after reviewing the plan.
4. Confirm `.framework/manifest.json` contains `project.archetype` and `project.mode`.
5. Remove any remaining workflow dependency on legacy standalone runtime configuration.
6. Register or promote the current active system with `assay system register ... --primary` or `assay system promote ...`.
7. Run `assay check --root <workspace>`.

## Dogfood verification

Verified on 2026-06-30 with the built CLI (`packages/assay-cli/dist/cli.js`) in a temporary workspace root.
