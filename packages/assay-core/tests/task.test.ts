...
function taskFile(root: string, id: string, overlay = false): string {
  return path.join(root, ...(overlay ? [".assay", "tasks"] : ["tasks"]), id, "task.json");
}
...
describe("native Task creation and Markdown contract", () => {
  it.each(["standalone", "overlay"] as const)(
    "creates a complete compatibility envelope and required PRD in %s mode",
    async (mode) => {
      const root = await workspace(`Create-${mode}`, mode);
      ...
      const raw = JSON.parse(
        await readFile(taskFile(root, created.task.id, mode === "overlay"), "utf8"),
...
describe("native Task persistence hardening", () => {
  ...
  it("recovers only an old lock owned by a dead pid", async () => {
    ...
    const lock = path.join(root, ".assay", "task-locks", created.task.id);
  ...
  it("never publishes an ownerless final lock when prepared-owner claim fails", async () => {
    ...
    const parent = path.join(root, ".assay", "task-locks");
  ...
  it("retries when a holder releases between owner inspection and owner read", async () => {
    ...
    const lock = path.join(root, ".assay", "task-locks", "release-race");
  ...
  it("fails closed on unknown lock ownership and releases only its own token", async () => {
    ...
    const unknown = path.join(root, ".assay", "task-locks", created.task.id);
    ...
    const changed = path.join(root, ".assay", "task-locks", "owner-change");
    await expect(
  ...
  it("reports a dangling context without hiding valid task rows", async () => {
    ...
    const valid = await createTask({ root, title: "Valid" });
    await writeFile(
      path.join(root, ".assay", "task-contexts.json"),
...
describe("native Task overlay conversion", () => {
  ...
  it.each([
  ...
  ])("$label preserves task directory bytes", async ({ move, keepOverlay }) => {
    const root = await workspace(`Convert-${String(move)}`, "overlay");
    ...
    expect(await readFile(path.join(target, ".assay", "task-contexts.json"))).toEqual(
      contextBefore,
    );
    expect((await showTask({ root: target, id: task.task.id })).task.title).toBe("History");
  });
  ...
  it("fails before writes when target tasks is nonempty", async () => {
    ...
    await expect(convertOverlayToStandalone({ root, target })).rejects.toThrow(
      /target tasks path already contains content/,
    );
    expect(await lstat(path.join(target, ".assay", "manifest.json")).catch(() => null)).toBeNull();
  });
  ...
  it("rejects a redirecting source task tree before conversion output", async () => {
    ...
    try {
      await symlink(outside, research, process.platform === "win32" ? "junction" : "dir");
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "EPERM") return;
      throw error;
    }
  ...
  it("rejects a redirecting target tasks directory before copy or move output", async () => {
    for (const move of [false, true]) {
      ...
      ).rejects.toThrow(/target tasks.*symlink|target tasks.*junction|target tasks.*reparse point/);
      expect(
        await lstat(path.join(target, ".assay", "manifest.json")).catch(() => null),
      ).toBeNull();
      expect(await readdir(outside)).toEqual([]);
      expect(await lstat(taskFile(root, task.task.id, true))).toBeDefined();
    }
  });

  it("preflights nonempty target context bytes before copy or move writes", async () => {
    for (const move of [false, true]) {
      const root = await workspace(`ConvertContextConflict-${String(move)}`, "overlay");
      const task = await createTask({ root, title: "Context history" });
      await bindTask({ root, contextKey: "session:context", id: task.task.id });
      ...
      // Even identical nonempty state is not silently merged or overwritten.
      await writeFile(targetContext, sourceContextBytes);
      const targetBefore = await readFile(targetContext);
      await expect(
        convertOverlayToStandalone({ root, target, move, keepOverlay: !move }),
      ).rejects.toThrow(/target task contexts already contain content/);
      expect(await readFile(sourceContext)).toEqual(sourceContextBytes);
      expect(await readFile(taskFile(root, task.task.id, true))).toEqual(sourceTaskBytes);
      expect(await readFile(targetContext)).toEqual(targetBefore);
      expect(
        await lstat(path.join(target, ".assay", "manifest.json")).catch(() => null),
      ).toBeNull();
    }
  ...
  it("rejects a redirecting target context ancestor without touching either tree", async () => {
    for (const move of [false, true]) {
      const root = await workspace(`ConvertContextRedirect-${String(move)}`, "overlay");
      const task = await createTask({ root, title: "Context redirect" });
      ...
      await mkdir(target);
      await mkdir(outside);
      const outsideContext = path.join(outside, "task-contexts.json");
      await writeFile(outsideContext, "outside\n", "utf8");
      try {
        await symlink(
          outside,
          path.join(target, ".assay"),
          process.platform === "win32" ? "junction" : "dir",
        );
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code === "EPERM") return;
  ...
  it("refuses conversion while a crash-left checkpoint transaction exists", async () => {
    for (const move of [false, true]) {
      const root = await workspace(`ConvertCheckpoint-${String(move)}`, "overlay");
      const task = await createTask({ root, title: "Checkpoint conversion" });
      const before = await readFile(taskFile(root, task.task.id, true));
      setTaskTransactionProbeForTests((stage) => (stage === "after-handoff" ? "crash" : undefined));
      await expect(
        checkpointTask({ root, id: task.task.id, handoff: HANDOFF }),
      ).rejects.toMatchObject({ code: "TASK_IO_ERROR" });
      setTaskTransactionProbeForTests(undefined);
      ...
      await expect(
        convertOverlayToStandalone({ root, target, move, keepOverlay: !move }),
      ).rejects.toThrow(/checkpoint transaction must be recovered before conversion/);
      expect(await readFile(taskFile(root, task.task.id, true))).toEqual(before);
      expect(await readFile(transaction)).toEqual(transactionBefore);
      expect(
        await lstat(path.join(target, ".assay", "manifest.json")).catch(() => null),
      ).toBeNull();
    }
