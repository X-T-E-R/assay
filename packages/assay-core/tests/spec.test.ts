...
describe("native Spec", { timeout: 60_000 }, () => {
  it("lazily creates storage, allocates across live/archive, and preserves body bytes", async () => {
    ...
    expect(
      await readFile(path.join(root, "project", "specs", "README.md")).catch(() => null),
    ).toBeNull();
  ...
  it("rejects unknown legacy contents and reparse redirects without overwriting", async () => {
    ...
    await mkdir(path.join(root, "project", "specs"));
  ...
  it("stores overlays under .assay and fail-closes mutation during byte-preserving conversion", async () => {
    ...
    expect(original.path).toBe(`.assay/project/specs/${original.item.id}`);
    await writeFile(bodyFile(path.join(root, ".assay"), original.item.id), validBody, "utf8");
    const originalItemBytes = await readFile(path.join(root, original.path, "spec.yaml"));
    const originalBodyBytes = await readFile(path.join(root, original.path, "specification.md"));
    const target = `${root}-standalone`;
    roots.push(target);
    let enteredResolve: (() => void) | undefined;
    let releaseResolve: (() => void) | undefined;
    const entered = new Promise<void>((resolve) => {
