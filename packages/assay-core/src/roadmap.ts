...
async function locationFor(rootInput: string): Promise<RoadmapLocation> {
  ...
  if (!manifest)
    throw new RoadmapError("WORKSPACE_NOT_FOUND", `No Assay manifest found at ${root}.`);
  ...
  const locksDirectory = path.join(root, layout.state_root, "roadmap-locks");
  try {
...
async function assertUnchanged(
  ...
  if (!sameIdentity(before.identity, currentInfo) || currentRaw !== before.raw) {
    throw new RoadmapError(
      "ROADMAP_CONFLICT",
      `roadmap item changed outside Assay: ${before.result.item.id}`,
    );
  }
}
