...
This directory contains Assay-native specifications. Each live Spec is stored at \`<id>/{spec.yaml,specification.md}\`; retired Specs may be moved unchanged to \`archive/<id>/\`.
...
async function locationFor(rootInput: string, tolerateSpecStorage = false): Promise<SpecLocation> {
  ...
  if (!manifest) throw new SpecError("WORKSPACE_NOT_FOUND", `No Assay manifest found at ${root}.`);
  ...
  const locksDirectory = path.join(root, layout.state_root, "spec-locks");
  const stagingDirectory = path.join(root, layout.state_root, "spec-staging");
...
async function assertRecordUnchanged(before: ReadSpec): Promise<void> {
  ...
  if (
    ...
    throw new SpecError("SPEC_CONFLICT", `spec changed outside Assay: ${before.result.item.id}`);
  }
}
...
async function assertGraphSnapshotsUnchanged(
  ...
  for (const before of snapshots) {
    try {
      ...
      throw new SpecError(
        "SPEC_CONFLICT",
        `spec graph changed outside Assay before publication: ${before.result.item.id}`,
        { cause: error },
      );
    }
  }
