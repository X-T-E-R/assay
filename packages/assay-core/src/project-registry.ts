...
export interface AssayProjectRecord {
  ...
  readonly createdBy: AssayProjectRegistryCommand;
  readonly lastCommand: AssayProjectRegistryCommand;
  readonly frameworkVersion?: string;
  readonly layoutVersion?: number;
  readonly managedFiles: number;
  readonly status: AssayProjectRegistryStatus;
}
...
type ProjectRecordBuild = Omit<AssayProjectRecord, "frameworkVersion" | "layoutVersion"> & {
...
export async function registerProject(
  projectPath: string,
  command: Exclude<AssayProjectRegistryCommand, "uninstall">,
...
export async function listProjectRecords(
  ...
  const refreshed = await Promise.all(
    records
      .filter((record): record is AssayProjectRecord => record !== null)
...
export async function scanForProjects(
  ...
  const found = new Map<string, AssayProjectRecord>();
...
async function buildProjectRecord(
  ...
  command: AssayProjectRegistryCommand,
  options: ProjectRegistryOptions,
  overrides: Partial<Pick<AssayProjectRecord, "status">> = {},
): Promise<AssayProjectRecord> {
...
function compactRecord(record: ProjectRecordBuild): AssayProjectRecord {
...
async function readProjectRecord(
  id: string,
  options: ProjectRegistryOptions,
): Promise<AssayProjectRecord | null> {
  if (!isProjectId(id)) {
    return null;
  }
...
async function readProjectRecordFile(filePath: string): Promise<AssayProjectRecord | null> {
  try {
...
async function refreshProjectRecordStatus(record: AssayProjectRecord): Promise<AssayProjectRecord> {
...
function isProjectRecord(value: unknown): value is AssayProjectRecord {
  if (!isObject(value)) {
    return false;
  }
  return (
...
function isRegistryCommand(value: unknown): value is AssayProjectRegistryCommand {
  return (
    value === "init" ||
    value === "adopt" ||
    value === "update" ||
...
function isRegistryStatus(value: unknown): value is AssayProjectRegistryStatus {
