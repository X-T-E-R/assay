...
async function tempDir(): Promise<string> {
  const root = await mkdtemp(path.join(tmpdir(), "metasystem-manifest-archetype-"));
  tempRoots.push(root);
  return root;
}
...
describe("manifest archetype/mode contract", () => {
  ...
  it("throws a typed error instead of preserving invalid manifest values", async () => {
    const root = await tempDir();
    await writeManifestJson(root, legacyManifest({ archetype: "metasystem" }));
